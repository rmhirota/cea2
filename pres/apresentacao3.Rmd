...
map_df(list(d1,d2,d3), tidy) %>% knitr::kable()
```
...
map_df(list(n1,n2,n3), tidy) %>% knitr::kable()
```
...
map_df(list(b1,b2,b3), tidy) %>% knitr::kable()
```
...
map_df(list(p1,p2,p3), tidy) %>% knitr::kable()
```
...
- Olhando a condição basal 2, todos os dias aparentam manter a mesma distribuição 
...
map_df(list(w1,w2,w3), tidy) %>% knitr::kable()
```
...
- Partindo para testes unicaudais, é possível verificar qual o sinal da comparação
entre as medianas dos dias 1 e 3, pois o teste com $H_1 : Md1 > Md3$ resultou em um valor-p = 0,07;
...
- Considerando todos os grupos como uma unidade, a probabilidade de ativar o vídeo pela primeira vez nos primeiros milisegundos é maior no dia 3;

- No entanto, a probabilidade de ativar o vídeo pela primeira vez após os 50 primeiros milisegundos é maior no dia 2;
...
- A hipótese de igualdade entre os grupos varia para cada condição;

- Na basal 2, não há indicios de diferença entre os grupos, ou seja, em todas as sessões parece existir uma mesma distribuição da frequência de apertos

### Primeira Ativação do vídeo (Contingente)

- Parece existir uma diferença no período da primeira ativação do vídeo entre os dias;

- Porém, entre os grupos o comportamento de cada sessão mantem-se o mesmo 
