...
.top-column[
```{r fig.weight = 4}
dia1 = cea2::da_spss %>% dplyr::filter(dia==1) %>% dplyr::select(condicao, grupo, n_apertos)
...
d12 <- data %>% 
  ggplot2::ggplot(ggplot2::aes(x = dia1, y = dia2, colour = grupo, shape = condicao)) +
  ggplot2::geom_point() +
  ggplot2::geom_abline(slope = 1, intercept = 0)
d23 <- data %>% 
  ggplot2::ggplot(ggplot2::aes(x = dia2, y = dia3, colour = grupo, shape = condicao)) +
  ggplot2::geom_point() +
  ggplot2::geom_abline(slope = 1, intercept = 0)
d13 <- data %>% 
  ggplot2::ggplot(ggplot2::aes(x = dia1, y = dia3, colour = condicao)) +
  ggplot2::geom_point() +
  ggplot2::geom_abline(slope = 1, intercept = 0)
...
da_dia <- cea2::da_spss %>% 
  ...
  dplyr::select(condicao, grupo, dia, n_apertos) %>% 
  dplyr::mutate(dia = dplyr::case_when(
    dia == 1 ~ "dia_1",
    dia == 2 ~ "dia_2",
    TRUE ~ "dia_3"
  )) %>% 
  tidyr::pivot_wider(dia)
