...
???
_foram observadas diferenças na força de preensão frente a um mesmo estímulo ambiental (DIONISIO, 2012); comportamento antecipatório (GREDEBÄCK et al., 2009; HOFSTEN; RÖNNQVIST,
1988) e ainda, atuação ativa com inibição de um comportamento estereotipado
(MEER; WEEL; LEE, 1995_
---
...
???
_Condições Contingentes e Não Contingente foram apresentadas em ordem aleatório para cada bebê_
---
## Sobre o experimento
...
???
_O estado de alerta do bebê durante o procedimento pode ser um fator que influência sua responsividade a estímulos e o cenário "perfeito" para a execução do teste seria quando o indivíduo estivesse quieto e acordado._
???
...
- *Pico*: pressão máxima (mmHg)  
  ...
  A pressão do pico dentro de cada aperto foi definida como o valor máximo exercido entre o início e o término do aperto  
  
- *Media_pressao_pico*
- *Media_duracao_aperto* (em milissegundos)
...
# grupo 2
cea2::da_tidy %>%
  dplyr::filter(condicao=="basal1"|condicao=="contingente",grupo == "2",pressao<4) %>%
  dplyr::mutate(Tempo = floor(tempo*10)/10) %>%
  dplyr::group_by(condicao, Tempo) %>%
  dplyr::summarise(`Média Pressão` = mean(pressao), .groups = "drop") %>%
  highcharter::hchart(
