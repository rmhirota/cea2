...
- **Ideia**: Avaliar se o tempo entre a ocorrência dos eventos se ajusta a uma distribuição exponencial;

- **Objetivo**: Associar a cada bebê um Processo de Poisson


Para o _i_-ésimo grupo, na _j_-ésima condição e no _k_-ésimo dia, teríamos

$$Tempo \; entre \; apertos_{ijk} \sim  Exponencial(\alpha_{ijk})$$
E um Processo de Poisson associado a cada bebê

$$Frequência \; de \; apertos_{ijkl} \sim  Poisson(\lambda_{ijk}t_{ijkl})$$

com $$t_{ij} = tempo \; da \; sessão$$
---
## Testes de exponencialidade

- **Gráfica**: Histogramas da variável _Tempo entre apertos_ para cada bebê;
...
**Ideia**: estimar 36 parâmetros $\theta_{i}$ e compará-los entre si através de testes de hipótese 

**Objetivo**: comparar as taxas de apertos por minuto dos grupos x condição x dia
\\
\\
...
Procedimento baseado em estimador por região (HPD)

<center><img src="imagens/area2.JPG" width="400" hight = "400"></center>
...
---
## Teste de Hipótese precisa e e-value
...
$$e-value = E_{v}(\Theta_{0},x) = 1 - P(\theta \in T_{x} | X = x)$$
com
...
---
## Teste para hipóteses compostas 
...
$$E_{v}[(\theta_{1},\theta_{2}),x] = 1 - P[(\theta_{1},\theta_{2}) \in T_{XY} | X, Y]$$
com

$$T_{XY} = [\theta = (\theta_{1},\theta_{2}) \in R^{2}_{+}: \pi(\theta_{1},\theta_{2} | X, Y) > \pi^{*}(\theta_{1},\theta_{2} | X, Y)]$$

<center><img src="imagens/area1.JPG" width="400" hight = "400"></center>
